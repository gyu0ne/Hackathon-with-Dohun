...
class AIHubStreamDataSet(Dataset):
    ...
    def _one_sample(self, sample_index: int) -> Any:
        ...
        if not text or crop is None:
            raise ValueError("indexed annotation is no longer a valid crop")
    ...
    def __getitem__(self, sample_index: int) -> Any:
        ...
        for _ in range(MAX_SAMPLE_ATTEMPTS):
            try:
                ...
                self.logger.error("streaming sample %s failed: %s", current, error)
            current = (current + 1) % len(self)
        raise RuntimeError(
            f"Could not load a valid streaming sample after {MAX_SAMPLE_ATTEMPTS} attempts"
        )
